...
<template>
    <h1 class="text-center my-3">Fresh Food</h1>
    <div class="card-grid">
        <div v-for="item in data" class="card my-2" :key="item.id">
            <div class="card-header d-flex justify-content-between ">
                <h3 style="width:208px ; height:35px ;" class="overflow-hidden">{{ item.nama_food }}</h3>
                <button type="button" class="btn btn-primary" @click="showPopup(item.id_food)">buy</button>
            </div>
            <div class="card-body">
                <div class="card-title">
                    Jenis Makanan: <h2 class=""> {{ item.jenis_fod }}</h2>
                </div>
...
export default {
    ...
    methods: {
        ...
        fetchSelect() {
            ...
            axios.get('http://localhost:8000/api/makanan/' + this.selectedMakanan)
                .then(response => {
